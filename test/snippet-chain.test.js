...
test("challenge parser captures hashcashX and segmentLen", () => {
  const args = extractChallengeArgs('<script>g("bind", 20, "ticket", "path", 3.5, 4, "r", "api")</script>');
  assert.ok(args, "challenge args extracted");
  assert.equal(args.hashcashX, 3.5);
  assert.equal(args.segmentLen, 4);
});
