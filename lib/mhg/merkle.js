...
export const leafHash = async (page) => {
  assertBytes("page", page);
  return digest(encoder.encode("mhg:leaf"), page);
};

export const nodeHash = async (left, right) => {
  assertBytes("left", left);
  assertBytes("right", right);
  return digest(encoder.encode("mhg:node"), left, right);
};
