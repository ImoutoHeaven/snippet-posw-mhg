...
export const verifyOpenBatchVector = async ({
  ...
  for (const open of opens) {
    ...
    const proofValidationCache = new Map();
    ...
    const ensureProofVerified = async (needIdx) => {
      if (proofValidationCache.has(needIdx)) {
        return proofValidationCache.get(needIdx);
      }
      const loaded = loadRequiredNode(needIdx);
      if (!loaded.ok) {
        const failure = loaded.reason === "missing_witness"
          ? { ok: false, reason: "missing_witness", index: loaded.index }
          : { ok: false, reason: "bad_open" };
        proofValidationCache.set(needIdx, failure);
        return failure;
      }
      ...
      const proofOk = await verifyProof({
        root: rootBytes,
        index: needIdx,
        page: loaded.node.page,
        proof: loaded.node.proof,
        leafCount,
      });
      const result = proofOk
        ? { ok: true, node: loaded.node }
        : { ok: false, reason: "proof_failed", index: needIdx };
      proofValidationCache.set(needIdx, result);
      return result;
    };
    ...
    for (const j of eqSet) {
      ...
      if (j === 1) {
        ...
        const p0Verified = await ensureProofVerified(p0);
        if (!p0Verified.ok) {
          if (p0Verified.reason === "bad_open") {
            return { ok: false, reason: "bad_open" };
          }
          return { ok: false, reason: p0Verified.reason, index: p0Verified.index };
        }
        const p1Verified = await ensureProofVerified(p1);
        if (!p1Verified.ok) {
          if (p1Verified.reason === "bad_open") {
            return { ok: false, reason: "bad_open" };
          }
          return { ok: false, reason: p1Verified.reason, index: p1Verified.index };
        }
        const p2 = await deriveDynamicParent2({
          ...
          p0,
          p1,
          p0Page: p0Verified.node.page,
          p1Page: p1Verified.node.page,
        });
        edge = { p0, p1, p2 };
      }
      equations[j] = edge;
    ...
    for (const needIdx of required) {
      const verified = await ensureProofVerified(needIdx);
      if (!verified.ok) {
        if (verified.reason === "bad_open") {
          return { ok: false, reason: "bad_open" };
        }
        return { ok: false, reason: verified.reason, index: verified.index };
      }
    }

    if (required.has(0)) {
      if (!expectedGenesis) {
        // Cache canonical genesis once, only when an open actually needs node 0.
        expectedGenesis = await makeGenesisPage({ graphSeed, nonce, pageBytes });
      }
      const genesisNode = decodedNodes.get(0);
      // Required-node loading should guarantee node 0 exists; fail closed if that invariant is broken.
      if (!genesisNode || !equalBytes(genesisNode.page, expectedGenesis)) {
        return { ok: false, reason: "equation_failed", index: 0 };
      }
    }
