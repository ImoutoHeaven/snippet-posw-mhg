...
const HASHCASH_W32_RANGE = 0x1_0000_0000;
...
const prefixU32be = (bytes) =>
  (((bytes[0] << 24) | (bytes[1] << 16) | (bytes[2] << 8) | bytes[3]) >>> 0);

const hashcashThresholdFromX = (value) => {
  const x = Number(value);
  if (!Number.isFinite(x) || x <= 1) return 0;
  return Math.max(1, Math.floor(HASHCASH_W32_RANGE / x));
};
...
const buildPowSample = async (config, powSecret, ticket, commitMac, sid) => {
  ...
  const hashcashThreshold = hashcashThresholdFromX(config.POW_HASHCASH_X);
  ...
  return { indices, segLensAll, hashcashThreshold };
};
...
const handlePowOpen = async (request, url, nowSeconds, innerCtx) => {
  ...
  const { indices, segLensAll, hashcashThreshold } = sample;
  const difficultyBinding = getPowDifficultyBinding(config);
  if (hashcashThreshold > 0 && !indices.includes(ticket.L)) return denyCheat();
  ...
  if (hashcashThreshold > 0 && expectedBatch.includes(ticket.L)) {
    ...
    if (prefixU32be(digest) >= hashcashThreshold) return denyCheat();
  }
