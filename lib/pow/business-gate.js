...
const verifyTicketMac = async (ticket, url, bindingValues, config, powSecret) => {
  ...
  const bindingString = makePowBindingString(
    ...
    difficultyBinding.eqN,
    difficultyBinding.eqK,
  );
...
const respondPowChallengeHtml = async (
  ...
  const bootstrap = {
    ticketB64,
    pathHash,
    eq: { n: difficultyBinding.eqN, k: difficultyBinding.eqK },
    apiPrefix: config.POW_API_PREFIX,
  };
  const bootstrapB64 = base64UrlEncodeNoPad(encoder.encode(JSON.stringify(bootstrap)));
